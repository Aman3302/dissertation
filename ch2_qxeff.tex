...
Broadly we find that dense, moderately short-lived AGN disks are preferred for producing an anti-correlation between the mass ratio $q$ and effective spin $\Xeff$ like has been identified from existing gravitational wave (GW) observations.
Additionally, a BH initial mass function (IMF $\propto M^{-2}$) is preferred over a more top-heavy IMF ($M^{-1}$), where $M$ is the BH mass.
...
\begin{figure}
	\centering
	\includegraphics[width=1\textwidth]{xeff-diagram-eqns.pdf}
...
Throughout this text we will refer to a BH that formed through stellar evolution processes as a first generation, or a 1g, BH.
Each merger event increases the generation of the highest generation component by one such that a 2g$-$4g merger event produces a 5g remenant.
...
For intuition in what follows, the range of $q$ among first generation (1g; BH produced via stellar evolution) mergers will depend on the range of the initial mass function (e.g., $10\leq M \leq 40\,\Msun$) as well as the power-law of the mass distribution ($\propto M^{-\gamma}$).
...
We use two common one-dimensional radial models, namely, \citet[][hereafter SG]{SirkoGoodman03} and \citet[][hereafter TQM]{ThompsonQuataertMurray05}.
...
In practice, mergers also aggregate near $R\sim10^3\,\Rg$ (for $\Mbh = 10^8\,\Msun$) when using migration prescriptions described by \citet{GrishinGilbaumStone24} as BH enter the region of lower migration speeds as they aproach the trap.
...
Likewise migration traps move radially outward in units of $\Rg$ as the SMBH mass drops well below $\Mbh 10^8\,\Msun$, but in Paper I we compared these models and found little change for $\Mbh = 10^8\,\Msun$ \citep[see Fig. 6 \& 7 of][]{McKernan+25-mcfactsI}, which is also the SMBH mass we find that produces the most mergers \citep{Delfavero+25}.
...
We draw locations from a broken power law distribution for the radial BH number density $n$ representing a ``cored'' type cluster where the radial number density index changes at a critical radius $R_{\rm crit}=0.25$ pc or $5\times10^4\,\Rg$ when $\Mbh=10^8\,\Msun$.
...
We note that the standard TQM model uses an SMBH mass of $10^9~\Msun$, but \citet{Delfavero+25} find $\sim 10^{8}\Msun$ is likely to be the dominant contributor to AGN channel merger rates.
...
\begin{tabular}{r|cc}
    ...
    \code{disk_radius_trap} & 700 & $\rm{r}_{g,\rm{SMBH}}$ \\
    \code{disk_radius_outer} & $5\times 10^{4}$ & $\rm{r}_{g,\rm{SMBH}}$ \\
    \code{disk_radius_max_pc} & 0 & \\
    \code{disk_alpha_viscosity} & 0.01 & \\
    \code{inner_disk_outer_radius} & $50$ & $\rm{r}_{g,\rm{SMBH}}$ \\
    \code{disk_inner_stable_circ_orb} & $6$ & $\rm{r}_{g,\rm{SMBH}}$ \\
    ...
    \code{disk_radius_capture_outer} & $10^{3}$ & $\rm{r}_{g,\rm{SMBH}}$ \\
...
\caption[Default \code{McFACTS} input parameters]{Default input parameters for the SMBH, AGN disk and NSC as well as physics choices in \texttt{p1\_model\_default.ini}.}
...
Our IMF range limits the mass ratio range of 1g-1g mergers to $q>$0.25, but see \citet{Delfavero+25} for a study producing mergers down to $q \sim 0.1$.

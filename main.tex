...
\include{title}  % Title Page - title.tex

\pagestyle{plain} % Now start showing page numbers

\include{committee}  % Grad School Committee Page - committee.tex (THIS REPLACES THE APPROVAL PAGE AS OF SPRING 2019) (e.g., Don't use the page in the math dept latex example thesis)

\include{dedication}  % Dedication Page - dedica.tex

\include{acknowledgements}  % Acknowledgements Page - ackno.tex

\include{vita}  % Vita page - vita.tex

\setstretch{1}%\doublespace

\setstretch{2}%\begin{doublespace}
\include{abstract}  % Abstract page - abstract.tex
...
\include{abbrev}  % Page that contains a list of abbreviations. This is optional, so if you don't have many or don't want it, just comment out this line
...
\clearpage
\resetcounters
\input{appendix_mcfacts}

\clearpage
\resetcounters
\input{appendix_ocotillo}
...
% \immediate\write\@mainaux{\string\bibstyle{aasjournal}}
% \immediate\write\@mainaux{\string\bibdata{refs}}

...
Active disks (those internally heated) are much hotter in the midplane and have much more complicatedvertical structures, and determining the $T(z)$ dependence is non-trivial.
...
This method outlined below forms the core the \code{Ocotillo} code, a multi-wavelength post-processing radiative transfer calculations, we developed for this purpose.
...
\begin{figure}
    \centering
    \includegraphics[width=1.0\columnwidth]{ch_snrt_figs/Feautrier_UVIpIm.png}
...
Fig. \ref{fig:feautrier_solution} shows the results from solving for $U(z)$ and deriving $V(z)$, $\Iplus(z)$, and $\Iminus(z)$ with differnet lines corresponding to wavelength using the procedure described above applied to a veritical slice through the disk in our fiducial model.
...
\begin{figure}
    \centering
    \includegraphics[width=1.0\textwidth]{ch_snrt_figs/opacity_w4473_0000.pdf}
...
\begin{figure}
    \centering
    \includegraphics[width=1.0\textwidth]{ch_snrt_figs/opacity_w4473_0013.pdf}
...
\begin{figure}
    \centering
    \includegraphics[width=1.0\textwidth]{ch_snrt_figs/opacity_w4473_0031.pdf}
...
The blue curve in Fig.~\ref{fig:all_lcs} is the light curve calculated using our post-processing radiative transfer code \code{Ocotillo}
...
While studying spectra for AGN accretion disks, \citet{Hubeny+01} found that for a disk around a black hole of mass $\Mbh=10^8\,\Msun$ with a viscocity parameter of $\alpha=0.01$ has a luminosity $L=0.3L_\text{Edd}=4.4\cdot10^{45}(M/\Msun)$ erg s$^{-1}$ when considering electron scattering.

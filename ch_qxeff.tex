...
\par The model and results described in this chapter were produced using the \code{McFACTS} code that was initially published in a series of papers \code{McFACTS I}
\citep{McKernan+25-mcfactsI}, \code{McFACTS II} \citep{Cook+25}, and \code{McFACTS III} \citep{Delfavero+25-mcfactsIII} in the \textit{Astrophysical Journal}.
...
Please cite \citet{McKernan+25-mcfactsI} for specifics model details and \citet{Delfavero+25-mcfactsIII} for results related to a cosmologically informed galaxy population.
...
The assumptions we vary include the BH initial mass function, disk model, disk size, disk lifetime, NSC model, and the prograde-to-retrograde fraction of newly formed black hole binaries.
...
Thus, when binaries form due to encounters along similar trajectories, $\hat{\vec{L}}_\text{bin}$ will preferentially align with the disk orbital angular momentum, causing $\Xeff$ to be more restricted than when no disk is present despite the fact that $\vec{\chi_i}$ for any newly captured BH will be randomly oriented, as shown in the right panels.
...
\subsection{The life of a black hole in \code{McFACTS}}
...
The default extent is $5 \leq R/\Rg \leq 5\times10^5$ with the alpha viscosity parameter $\alpha=0.01$, and a 
lifetime depending on the disk model (see below).


One of my contributions to \code{McFACTS} was to implement a mechanism to draw initial BH locations in the disk by selecting one of the following probability distributions: 1) uniform distribution 2) broken power-law distribution.
...
Our NSC model is defined with a broken powerlaw number density profile that goes as $n\propto R^{-7/4}$ out to the critical radius at $R\sim 0.25$ pc and $n\propto R^{-2.5}$ beyond that to $5$ pc and contains $10^4$ BHs within the central $\text{pc}^3$ \citep{Generozov+18}.
...
The final probabilty density distribution is shown in \Fig{fig:nsc-loc-sampling} by the purple shaded region alongside those for the uniform distribution (red) and unscaled broken power-law distribution (teal).
Also plotted are histograms of the resulting draws for 100 galaxies where the disk radius is $\Rout=10^6\,\Rs$, replicating the initial samples for a typical run in \code{McFacts}.
The radius is extended here beyond what of our studies to demonstrate the shape of the broken power law distribution and the BH populations in comparison with the uniform distribution.
...
The $y$-axis trimmed to ignore most of these to highlight the complexity in the outer disk.
...
The initial BH spins are drawn from a Gaussian centered on dimensionless spin parameter $\chi=0$ and width $0.2$, and the radial locations are drawn from a volume-normalized probability distribution according to the NSC powerlaw structure.
Finally, the orbital eccentricity is drawn from a uniform distribution in $e^2$ over the range $0<e<\emax$ and randomly assign whether the orbit is prograde or retrograde.
...
\centering
	...
	\caption[Sampling black holes from a nuclear star cluster]{Probability densities and resulting population of BH drawn from a NSC for a uniform distribution (red, solid line), a broken power-law (teal, dashed line), and the same power-law with proper volume scaling (purple, dash-dotted line).}
...
While most binaries will likely form from the circularized population, they can still interact with BHs on eliptical or inclined orbits.
Gas effects and dynamical encounters with a third BH can cause the binary to lose energy (harden) or gain energy (soften) until they become sufficiently hardened and merge.
...
We draw the initial BH masses from a Pareto distribution with $P(M) \propto M^{-\gamma}$ for $10\leq M/\Msun \leq40$, with an additional Gaussian component centered on $35M_{\odot}$ \citet[see][]{McKernan+25-mcfactsI}.

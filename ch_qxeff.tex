...
\section{\texorpdfstring{\MakeUppercase{AGN influence on the mass ratio and effective spin plane}}{AGN influence on the mass ratio and effective spin plane}}
\label{sec:qXeff}

\counterwithin{figure}{section}
\counterwithin{table}{section}

\par The model and results described in this chapter were published in the \textit{Astrophysical Journal} \citep{Cook+25}.
The main text of this chapter is directly informed by the content of \citet{Cook+25}, but please cite \citet{McKernan+25-mcfactsI} for model details and \citet{Delfavero+25} for results related to a cosmologically informed galaxy population.
...
Here we present a study using the \code{McFACTS} code \citep[Monte Carlo For AGN Channel Testing and Simulations:][]{McKernan+25-mcfactsI} to test the predictions of the AGN channel for the $q$--$\Xeff$ distribution across a wide range of parameter space in both AGN disk and NSC models, with a view to tightly constraining requirements on the AGN channel.
...
The left panel shows this for binary stellar companions whose orbits are close enough to experience periods of common-envelope evolution, that cause there orbits to shrink enough for GW radiation to shrink their orbits further.
In this scneario it is most likely that both stars formed from the same molecular cloud and have the same rotational angular momentum which restricts $\Xeff$ to be $>0$.
...
As discussed in the previous chapter, the presense of an accretion disk will align orbits of NSC objects to be fully embedded within its confines over time.
Thus, when binaries form due to encounters along similar trajectories, $\hat{L}_\text{bin}$ will preferentially align with the disk orbital angular momentum, causing $\Xeff$ to be more restricted than when no disk is present despite the fact that $\chi_i$ for any newly captured BH will be randomly oriented, as shown in the right panels.
Since merger remnants primarily inherit their spins from $L_\text{bin}$, $\Xeff$ becomes progressively more aligned or anti-aligned with the disk depending on whether both prograde $\L_\text{bin} \cdot \L_\text{disk} = +1$ and retrograde $\L_\text{bin} \cdot \L_\text{disk} = -1$ or only prograde binaries form.
...
For details of the code and further assumptions made therein see the discussion in Appendix~\ref{sec:apdx-mcfacts}.
...
In the \code{Makefile}, change the \code{SEED} parameter to the value listed in Table~\ref{tab:run-parameters} and .

...
Since merger remnants primarily inherit their spins from $\hat{\Lbin}$, $\Xeff$ becomes progressively more aligned or anti-aligned with the disk depending on whether both prograde $\hat{\Lbin} \cdot \hat{\Ldisk} = +1$ and retrograde $\hat{\Lbin} \cdot \hat{\Ldisk} = -1$ or only prograde binaries form.
...
Drawing spins ($\vec{\chi}_{1,2}$) from a uniform distribution between those that are aligned and anti-aligned with $\hat{\Lbin}$ means that the resulting $\Xeff$ distribution should be centered on $\Xeff \sim 0$, with the width of that distribution a function of the initial spin magnitudes $|\vec{\chi}_{1,2}|$; see, e.g., \cite{Callister21} for details.

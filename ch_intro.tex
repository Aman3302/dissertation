...
The radiation from the broad line region (BLR) in the rapidly moving accretion disk and narrow line region (NLR) associated with clouds of slowly moving gas in its vacinity produce broad and narrow spectral features, respectively.
...
Fast-spinning SMBH with strongly magnetized disk can produce jets that eject material with enough kinetic energy to leave the galaxy or galaxy cluster entirely \citep{Oei+24}.
...
In the future, the Laser Interferometer Space Antenna \citep[LISA,][]{LISA17,LISA23} will enable observations of extreme-mass ration inspirals (EMRIs) and intermediate-mass ratio inspirals \citep[IMRIs,][]{Speri+26} which are comprised of single stellar mass BHs that emit long-lived GWs as they approach the event horizon of a SMBH, which emit at the $\sim10^{-4}-10^{-1}$~Hz range.
...
In the AGN channel, this anti-correlation can arise due to several biases \citep{McKernan+22:q-Xeff,Santini+23, DittmannDempseyLi24}.
...
Thus, LVK O4 results can directly test gas hardening efficiency in AGN channel models, and we can provide context for those observations by exploring the influence changes to the disk and NSC paramters or specifics of binary formation have on the estimated properties of the GW emission using population studies.
...
Among these are phenomena known as quasi-periodic oscillations (QPOs), quasi-periodic eruptions (QPEs), tidal disruption events (TDEs), changing-look AGN caused by variability in the system, and anomalous flares.
...
They dismissed a supernova (SN) as the source of the flare because it did not show any color evolution as is typical of SNe light curves \citep{Hueichapan+25}.
...
We looked to an existing code, \code{RADMC3D}, which uses a Monte Carlo algorithm to solve the propagation of photon packets, but, being designed for protoplanetay disks, it did not have a structure that could be easily modified to accept the temperature-dependent opacities we needed to use to capture the AGN disk and hot SN ejecta.
We tried directly integrating the radiative transfer equation with AGN opacitites, but that approach suffered from poor flux conservation.
We found instead that an implicit algorithm, the Feautrier method, afforded us greater accuracy, and developed our own code to implement this approach, called \code{Ocotillo} (we thank Shane Davis for suggesting this method).
\code{Ocotillo} simultaneously calculates the wavelength-dependent mean intensity, flux, and continuous opacity and applies it to the output from our SN models to retrive integrated light curves and color evolutions.



\noindent We proceed to answer these questions using these methods in the following chapters.
